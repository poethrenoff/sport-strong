...
{else}
	<span class="red_text">Поля, отмеченные *, обязательны к заполнению.</span>
{/if}
	<form id="callback_form" action="/callback.php" method="post">
		...
		<div class="clear"></div>
		<p><label for="">Комментарий<br>(например, интересующий товар)</label> <textarea rows="" cols="" name="callback_comment">{$smarty.post.callback_comment|escape}</textarea></p>
