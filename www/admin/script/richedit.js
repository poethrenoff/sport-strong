/*
 +-------------------------------------------------------------------+
 |                 J S - R I C H E D I T   (v1.18)                   |
 |                                                                   |
 | Copyright Gerd Tentler               www.gerd-tentler.de/tools    |
 | Created: Jun. 2, 2003                Last modified: Feb. 10, 2007 |
 +-------------------------------------------------------------------+
...
===========================================================================================================
 ...
 9.0 on Windows, IE 5.2 and Safari 1.0 on Mac OS. However, the script works with them, too - a simple
 textarea will replace the richtext editor.
===========================================================================================================
*/
...
function Editor( areaName ) {
//---------------------------------------------------------------------------------------------------------
// Configuration
//---------------------------------------------------------------------------------------------------------
  this.areaName = areaName;                     // имя поля формы

  this.editorBGColor = "#f0f0f0";               // editor background color
  this.editorBorder = "1px solid #aaaaaa";      // editor border (CSS-spec: "size style color")

  this.textWidth = 460;                         // text field width (pixels)
  this.textHeight = 220;                        // text field height (pixels)
  this.textBGColor = "#ffffff";                 // text field background color
  this.textBorder = "1px solid #aaaaaa";        // text field border (CSS-spec: "size style color")
  ...
  this.iconPath = "/admin/image/editor";        // path to icons
...
//---------------------------------------------------------------------------------------------------------
  ...
  this.initEditor = function(content) {
    if(this.editor = this.getEditor()) {
      ...
      for(var i = document.forms.length - 1; i > 0 && !this.field; i--) {
        if(document.forms[i].elements[this.areaName]) {
          this.field = document.forms[i].elements[this.areaName];
          this.field.setAttribute( 'errors', document.getElementById(content).getAttribute( 'errors' ) );
        }
      }
  ...
  this.store = function(view) {
    if(this.field) {
      if(this.source) {
      	this.editor.document.body.innerHTML = this.field.value;
      }
      else {
        this.field.value = this.editor.document.body.innerHTML;
      }
    }
  }
  ...
  this.buildEditor = function() {
    ...
    document.writeln('<table border=0 cellspacing=1 cellpadding=0><tr align=center>' +
                     '<td><select style="font:menu" onChange="rto[' + this.id + '].doCmd(\'formatBlock\', this[this.selectedIndex].value); this.selectedIndex=0">' +
    ...
    document.writeln('<td><div style="width:1px; height:15px; border-left:1px solid #aaaaaa"></div></td>');
    ...
    document.writeln('<div style="margin-top: 2px; border-bottom:1px solid #aaaaaa"></div>');

    document.writeln('<table border=0 cellspacing=2 cellpadding=0><tr align=center>' +
                     '<td><select style="font:menu" onChange="rto[' + this.id + '].doCmd(\'fontName\', this[this.selectedIndex].value); this.selectedIndex=0">' +
                     ...
                     '</select></td>');
    document.writeln('<td><select style="font:menu" onChange="rto[' + this.id + '].doCmd(\'fontSize\', this[this.selectedIndex].text); this.selectedIndex=0">' +
    ...
    document.writeln('<textarea name="' + this.areaName + '" id="' + this.fieldName + this.id + '" class="cssSource' + this.id + '" wrap="virtual" onfocus="rto[' + this.id + '].editorFocus()"></textarea>');
  ...
  this.create = function(content) {
    ...
    if(rto[this.id] = this) {
      if((IE || GK) && DM) {
        ...
        document.write('<textarea name="' + this.areaName + '" class="simple"' + ' wrap="virtual"' +
        	' errors="' + document.getElementById(content).getAttribute( 'errors' ) + '">' +
        	document.getElementById(content).innerHTML + '</textarea>');
      }
      document.getElementById(content).removeAttribute( 'errors' );
    }
...
function buildColorChart(mode) {
  ...
  var html = '<table border=0 cellspacing=1 bgcolor=#808080>';
  var style, i, j;
  ...
  for(i = 0; i < c.length; i++) {
    ...
    for(j = 0; j < c[i].length; j++) {
      style = 'width:14px; height:14px; font-size:1px; cursor:pointer; background-color:#' + c[i][j];
      html += '<td width=14 height=14 bgcolor=#' + c[i][j] + ' style="padding: 0px">' +
